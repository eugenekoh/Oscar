...
from transformers.pytorch_transformers.modeling_bert import (BertEmbeddings,
                                                             ...
                                                             BertPredictionHeadTransform, BertModel)
...
class PersonalityBertEmbeddings(BertEmbeddings):
    def __init__(self, config):
        super().__init__(config)
        self.personality_embedding = nn.Embedding(215, config.hidden_size)
        self.p_fc = nn.Linear(config.hidden_size * 2, config.hidden_size)
        self.p_layernorm = BertLayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.p_dropout = nn.Dropout(config.hidden_dropout_prob)

    def forward(self, personality_ids, input_ids, token_type_ids=None, position_ids=None):
        bert_embeddings = super().forward(input_ids, token_type_ids, position_ids)
        personality_tensor = self.personality_embedding(personality_ids)
        personality_repeat = personality_tensor.unsqueeze(1).repeat(1, bert_embeddings.shape[1], 1)

        x = torch.cat((bert_embeddings, personality_repeat), -1)
        x = self.p_fc(x)
        x = self.p_layernorm(x)
        x = self.dropout(x)
        return x
...
class BertImgModel(BertPreTrainedModel):
    ...
    def forward(self, personality_ids, input_ids, token_type_ids=None, attention_mask=None,
        ...
        if head_mask is not None:
            ...
            head_mask = head_mask.to(
                dtype=input_ids.dtype)  # switch to fload if need + fp16 compatibility
        else:
            head_mask = [None] * self.config.num_hidden_layers

        embedding_output = self.embeddings(personality_ids, input_ids, position_ids=position_ids,
...
class BertForPersonalityImageCaptioning(CaptionPreTrainedModel):
    ...
    def encode_forward(self, personality_ids, input_ids, img_feats, attention_mask, masked_pos, masked_ids=None,
        ...
        outputs = self.bert(personality_ids, input_ids, img_feats=img_feats, attention_mask=attention_mask,
